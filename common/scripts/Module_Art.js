...
g_properties.add_properties(
    {
        ...
        art_title_font: ['user.art.title_font', 'Noto Sans SemiBold'],
        art_title_font_size: ['user.art.title_font_size', 24],
        art_box_font: ['user.art.box_font', 'Noto Sans'],
        art_box_font_size: ['user.art.box_font_size', 14]
    }
);
...
var g_art_module_fonts = {
    title: gdi.Font(g_properties.art_title_font,
                    g_properties.art_title_font_size),
    box: gdi.Font(g_properties.art_box_font,
                  g_properties.art_box_font_size)
};

/** @type {Object<string, number>} */
var g_art_module_colors = {};
g_art_module_colors.title = g_theme.colors.panel_text_dim;
...
//public:
    ...
    this.get_album_art_done = function (metadb, art_id, image, image_path) {
        ...
        if (!image) {
            ...
            if (!currentAlbum.length || currentAlbum === _tf(g_properties.group_format_query, metadb)) {
                var is_embedded = image_path.slice(image_path.lastIndexOf('.') + 1) === _tf('$ext(%path%)', metadb);
    ...
    this.get_album_art = function () {
        ...
        if (_tf('[' + g_properties.group_format_query + ']', metadb) === '') {
            currentAlbum = '';
        }
        else {
            currentAlbum = _tf(g_properties.group_format_query, metadb);
        }
        ...
        if (art_arr[cur_art_id]) {
            ...
            if (has_photoshop) {
                parent_menu.append_item(
                    ...
                    function () {
                        _runCmd('Photoshop ' + '\"' + art_arr[cur_art_id].path + '\"');
                    },
                    {is_grayed_out: art_arr[cur_art_id].is_embedded}
                );
            }
            ...
            parent_menu.append_item(
                'Open image folder',
                function () {
                    _explorer(art_arr[cur_art_id].path);
                },
                {is_grayed_out: !utils.IsFile(art_arr[cur_art_id].is_embedded)}
            );
        }
...
function Thumbs(cover_switch_callback_arg) {
    ...
    this.create_thumb_objects = function () {
        ...
        this.btns.buttons.front = new _button(x, y, w, h, default_thumb_imgs.front, function () {cover_switch_callback(0);}, 'Front');

        x += (is_vertical ? 0 : (w + p));
        y += (is_vertical ? (w + p) : 0);
        this.btns.buttons.back = new _button(x, y, w, h, default_thumb_imgs.back, function () {cover_switch_callback(1);}, 'Back');

        x += (is_vertical ? 0 : (w + p));
        y += (is_vertical ? (w + p) : 0);
        this.btns.buttons.cd = new _button(x, y, w, h, default_thumb_imgs.cd, function () {cover_switch_callback(2);}, 'CD');

        x += (is_vertical ? 0 : (w + p));
        y += (is_vertical ? (w + p) : 0);
        this.btns.buttons.artist = new _button(x, y, w, h, default_thumb_imgs.artist, function () {cover_switch_callback(3);}, 'Artist');
    };
    ...
    this.create_thumb_image = function (bw, bh, art_img, state, btnText) {
        ...
        switch (state) {//0=normal, 1=hover, 2=down;
            case 0:
                ...
                break;
            case 1:
                g.DrawRect(x, y, w - 1, h - 1, 1, g_pl_colors.group_title);
                break;
            case 2:
                g.DrawRect(x, y, w - 1, h - 1, 1, g_theme.colors.panel_text_dim);
                break;
        }
