...
function _text(mode, x, y, w, h) {
	...
	this.paint = (gr) => {
		if (this.lines.length) {
			...
			gr.DrawString("No text to display", gdi.Font("Noto Sans Semibold", 24, 0), _RGB(70, 70, 70), this.x, this.y, this.w, this.h, SF_CENTRE);
		}
	...
	this.metadb_changed = () => {
		...
		if (panel.metadb) {
			...
			case 'text_reader':
				...
				if (utils.IsDirectory(this.filename)) { // if folder, use first txt/log file
	...
	this.rbtn_up = (x, y) => {
		...
		panel.m.AppendMenuItem(utils.IsFile(this.filename) || utils.IsDirectory(this.filename) ? MF_STRING : MF_GRAYED, 1999, 'Open containing folder');
		panel.m.AppendMenuSeparator();
	}
